...
public:
  H264or5VideoStreamParser(int hNumber, H264or5VideoStreamFramer* usingSource,
			   FramedSource* inputSource, Boolean includeStartCodeInOutput, double frameRate = 0.0);
...
::H264or5VideoStreamFramer(int hNumber, UsageEnvironment& env, FramedSource* inputSource,
			   Boolean createParser, Boolean includeStartCodeInOutput, double frameRate, double parseFrameRate)
  ...
  fParser = createParser
    ? new H264or5VideoStreamParser(hNumber, this, inputSource, includeStartCodeInOutput,parseFrameRate)
    : NULL;
  fNextPresentationTime = fPresentationTimeBase;
  fFrameRate = frameRate; // We assume a frame rate of 25 fps, unless we learn otherwise (from parsing a VPS or SPS NAL unit)
}
...
::H264or5VideoStreamParser(int hNumber, H264or5VideoStreamFramer* usingSource,
			   FramedSource* inputSource, Boolean includeStartCodeInOutput, double frameRate)
  : MPEGVideoStreamParser(usingSource, inputSource),
    fHNumber(hNumber), fOutputStartCodeSize(includeStartCodeInOutput ? 4 : 0), fHaveSeenFirstStartCode(False), fHaveSeenFirstByteOfNALUnit(False), fParsedFrameRate(frameRate),
