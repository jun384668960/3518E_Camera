...
typedef struct tagSAMPLE_AENC_S
{
    ...
    FILE    *pfd;
    HI_BOOL bSendAdChn;
	void (*call_back)(HI_S32, AUDIO_STREAM_S *);
} SAMPLE_AENC_S;
...
void *SAMPLE_COMM_AUDIO_AencProc(void *parg)
{
    ...
    {     
        ...
        if (FD_ISSET(AencFd, &read_fds))
        {
			...
			if(pstAencCtl->call_back != NULL)
			{
				(pstAencCtl->call_back)(pstAencCtl->AeChn, &stStream);
			}
			else
			{
...
* function : Create the thread to get stream from aenc and send to adec
******************************************************************************/
HI_S32 SAMPLE_COMM_AUDIO_CreatTrdAencUse(AENC_CHN AeChn, ADEC_CHN AdChn, void (*call_back)(HI_S32, AUDIO_STREAM_S *))
{
    SAMPLE_AENC_S *pstAenc = NULL;
